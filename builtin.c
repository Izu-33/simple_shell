...
/**
 * get_builtin - Matches a command with a corresponding shellby function.
 ...
 * Otherwise exit with the given status value.
 */
int shellby_exit(char **args, char **front)
{
	int i, len_of_int = 10;
...
/**
 * shellby_cd - Changes the current directoryof the shell process.
...
/**
 * shellby_help - Displays info about shell builtin commands.
 * @args: An array of arguments.
 * @front: A pointer to the start of args.
 * Return: -Q if an error occurs. Otherwise 0.
 */
int shellby_help(char **args, char __attribute__((__unused__)) **front)
{
	if (!args[0])
